...
function writeGM(gmObject: gm.State, filename: string) {
  ...
  return new Promise((res: Resolver, rej: Resolver) =>
...
async function writeAudio(orgFilePath: string, destFileName: string) {
  ...
  return {
    duration,
    url: url.replace('https://hipod-dev2.s3.ap-northeast-2.amazonaws.com/audios', 'https://audio.hipod.co.kr'),
  };
}
...
export const uploadHandler = (fieldData: { [key: string]: FIELD_TYPE } = FIELD_DATA_DEFAULT) =>
  async(async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    async function createPromise(file: any) {
      const promise = [] as Promise<UPLOAD_DATA>[];
      const fileName = randToken.generate(32);

      if (fieldData[file.fieldname] === FIELD_TYPE.IMAGE) {
        promise.push(wrapper(file.fieldname, writeGM(gm(file.path).flatten().autoOrient().resize(1280, 1280, '>').noProfile(), fileName + '.jpg')));
        promise.push(wrapper(file.fieldname, writeGM(gm(file.path).flatten().autoOrient().resize(256, 256, '>').noProfile(), fileName + '.thumb.jpg')));
      }

      if (fieldData[file.fieldname] === FIELD_TYPE.AUDIO) {
        promise.push(wrapper(file.fieldname, writeAudio(file.path, fileName + '.mp3')));
      }
